...
export const storeWeatherAction = () => async (dispatch) => {
  try {
    ...
    const { data } = await axios
      ...
      .get(`https://api.openweathermap.org/data/2.5/onecall?lat=${latitude}&lon=${longitude}&exclude=hourly,minutely&appid=632062b391f82bc75255755dbb7eb685`)
    console.log(data)
    let weatherInfo = localStorage.setItem("weather",data)  
  
    dispatch({ type: actionTypes.GET_WEATHER_REQUEST, payload: weatherInfo });
  });
  } catch (error) {

  ...
  const getWeather =  () => {
    navigator.geolocation.getCurrentPosition(async(success) => {
      ...
      console.log(longitude, latitude); //This works too!
       ...
       await axios.get( //Works now! Apparently there is a ~10min wait before you can use API Key. Also it doesn't like API variable for some reason
          `https://api.openweathermap.org/data/2.5/onecall?lat=${latitude}&lon=${longitude}&exclude=hourly,minutely&appid=${API_KEY}`
        )
        .then((res) => res.json().then(data => {
            console.log(data)
            setTimeZoneEl(data.timezone)
        }
